    ...
    try {
      const bulkOps = books.map((book, index) => {
        ...
        return {
          updateOne: {
            filter: { _id: book._id },
            update: { $set: { code: `${prefix}-${sequenceNumber}`, bookCode: bookCode._id } },
          },
        };
      });
